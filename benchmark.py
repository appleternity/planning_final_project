...
def abstract(graph, num_p, version=""):
    ...
    with open("M{}.pkl".format(version), 'wb') as outfile:
        pickle.dump(M, outfile)
    
    with open("gamma{}.pkl".format(version), 'wb') as outfile:
...
def search(num_p, filename, name=""):
    ...
    # "0": "{32, 1}",
    with open("M{}.pkl".format(name), 'rb') as infile:
        M = pickle.load(infile)
    
    with open("gamma{}.pkl".format(name), 'rb') as infile:
        gamma = pickle.load(infile)
    """
    ...
    while container:
        ...
        if not region:
            ...
            real_path = refinement(S, M, graph, gamma, path[::-1], num_p)
...
def refinement(S, M, graph, gamma, path, num_p):
    ...
    for index, (S1, S2) in enumerate(zip(path[:-1], path[1:])):
        abs1, L1 = S1
        abs2, L2 = S2
...
def main():
    ...
    filename = "state/_ladder_k2_w1_state.json"
    graph = load_graph(filename)
...
def run():
    ...
    for num_p in range(1, 11):
        print("building abstract for p =", num_p)
        graph = load_graph(filename)
        abstract(graph, num_p)

        print("search for result")
        path, real_path = search(num_p, filename, "")
        print(path, real_path)
        if path is None or real_path is None:
            print("==> no result")
            continue

        else:
            t2 = datetime.now()
            time_used = (t2-t1).total_seconds()
            with open("ans_benchmark{}.txt".format(filename.split("/")[-1][:-5]), 'w') as outfile:
                outfile.write(filename + "\n")
                outfile.write("Minimum pursuers is N = {}\n".format(num_p))
                outfile.write("path:\n")
                outfile.write("\n".join(str(p) for p in real_path))
                outfile.write("\nTime elapsed: {}".format(time_used))
            quit()
